...
![food11 registered model, version 1](screenshots/lab3-ss/lab3-q1-model-versions.png)
...
The fixed stages (`Staging`, `Production`, `Archived`) were replaced by **aliases**: free-form names chosen by the user, such as `champion` or `challenger`, each pointing to one model version. We assigned `champion` to version 1 with:
...
![Version 1 with the champion alias](screenshots/lab3-ss/lab3-q2-champion-alias.png)
...
To serve a newer version, no code change and no image rebuild are needed. We would register the new model, which becomes version 2, move the alias with `client.set_registered_model_alias("food11", "champion", 2)`, and restart the service. The restart is needed because `serve.py` loads the model once at startup.
...
The full build, which had to run `uv sync`:
...
Every dependency step was `CACHED`; only `COPY src/` ran.

 Between two builds, the official `python:3.11-slim` tag was updated upstream, so Docker resolved it to a new digest (`sha256:e41613d4...` instead of `sha256:da047cb8...`). Since every layer sits on top of the base image, all the cached layers were invalidated, even `pip install uv`:
...
We pinned the base image to the digest already built with (`python:3.11-slim@sha256:da047cb8...`) to get stable caching.
...
The multi-stage image is only about 30MB smaller in content size (3.25GB vs 3.28GB). CONTENT SIZE is compared because the DISK USAGE column also counts the unpacked copy Docker keeps for running containers. `docker history` of the multi-stage image shows why the gap is small:
...
The `.venv` layer (6.2GB uncompressed) is almost the entire image; the base Debian and Python layers are about 140MB combined, and the code is 49kB. Multi-stage only removes build tooling from the final image, here `uv` and pip's leftovers, which is tens of MB. The virtual environment has to be in the final image either way, and it is dominated by `torch` with its CUDA libraries. The build log shows the largest downloads:
...
Without `.dockerignore` building the naive image sent the entire project folder to the Docker daemon:
...
![Docker Desktop crash during the build without .dockerignore](screenshots/lab3-ss/lab3-q6-docker-crash.png)

After restoring the `.dockerignore`, the same naive build sent almost nothing in no time:
...
For **image size**, the effect depends on the Dockerfile. In the naive one, `COPY . .` would copy everything sent into the image: the datasets in `data/` and `local_data/`, `mlruns/`, `mlflow.db`, `.git/`. In the multi-stage one, only specific files are copied (`pyproject.toml`, `uv.lock`, `src/`). With the `.dockerignore` temporarily disabled (renamed to `.dockerignore.bak`), building the multi-stage image transferred only 420B of context. Because this Dockerfile copies only specific files, its image size is not affected:
...
The folder most likely to **break** the build is `.venv/`. It is the host's virtual environment, created on Windows, with Windows executables and a `pyvenv.cfg` pointing to a Windows Python interpreter, none of which can run in a Linux container. Whether it breaks the build depends on the order of the steps: if it is copied *after* the environment is built (for example `uv sync` followed by `COPY . .`), it overwrites the Linux `.venv` and the container cannot start. In `Dockerfile.naive`, `COPY . .` runs *before* `uv sync`, so uv would most likely detect the invalid environment and recreate it, at the cost of a larger build context. The multi-stage Dockerfile never copies it at all. The data folders don't break correctness, but in this case their size is what crashed Docker Desktop. `mlflow.db`, `mlruns/` and `.git/` add size and can expose private data, but don't break the build.
...
We restarted the server listening on all interfaces and allowing that host:
...
Because my artifact store is a local folder, the server returns a Windows file path (`file:///C:/Users/eabou/.../mlruns/...`), and the container tries to read that path from its own filesystem, where it doesn't exist. It was fixed by bind-mounting my `mlruns/` folder into the container at that same path:
...
Yes. After shutting everything down (including a full restart of my machine), I started the mlflow server again and a new container from the same image, without rebuilding. The model loaded and gave the same prediction:
...
This shows that the image contains the **code and its environment** (Python, the dependencies in `.venv`, `serve.py`), but **not the model**. The model is fetched at runtime, at startup, from the registry through `models:/food11@champion`. A new container always loads whatever version `champion` points to at that moment, so the model can be updated without rebuilding the image. 
...
The missing pieces before another machine could reliably pull and run the exact same image are:

- **A container registry.** The image exists only in the local Docker installation. It has to be pushed to a registry (Docker Hub, GitHub Container Registry, ...) so other machines can pull it.
...
- **Automated builds.** The image should be built by a CI pipeline from the committed Dockerfile, not by hand on a laptop, so that a given commit always produces the image that gets deployed. Pinning the base image digest is part of this: the `python:3.11-slim` tag changed between two builds during this lab.
- **Shared runtime dependencies.** The container doesn't contain the model; it needs a tracking server and an artifact store at startup. The current setup relies on an mlflow server running on a laptop, a local artifact folder, and a bind mount of a Windows path, none of which exist on a CI runner or a Kubernetes cluster. It would need a tracking server reachable from the cluster, a shared artifact store (such as S3 or MinIO), and the tracking URI and credentials passed as environment variables and secrets.
