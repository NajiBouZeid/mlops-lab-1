...
Before settling on Solution 1, I also tried **Solution 2 (small sample data)** first, pushing a small subset of the dataset to DagsHub to confirm the git+dvc mechanism was working correctly. That sample is still visible on DagsHub under the Datasets tab, though my current `data.dvc` pointer now resolves to the full dataset tracked via Solution 1.
